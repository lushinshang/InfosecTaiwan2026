...
│   └── 簡報/                          # 講者簡報 PDF（共 4 份，含 2 份主議程與 2 份社群場簡報，後者於 HTML「補充資料」區塊收納）
...
Day 2 自己也踩出兩個新坑（來源檔案中途換版本、括號配對錯誤導致清稿腳本吃掉後半段內容），顯示「參考前一天的教訓」能省掉已知的坑，卻不保證不會有新坑——**每次重跑流程前，還是要對這次的輸入資料本身先做基本檢查，不能只靠複製上次的做法**。

**7. 簡報移入與 GitHub Pages 的中文檔名編碼坑**
- **動作**：將四份新收集的 Day 2 簡報 PDF 檔案移動至 `Day2/簡報/` 底下，並在 Day 2 網頁的「補充資料」區塊以時間順序排入。
- **遇到的坑**：在 macOS 本地端上傳包含中文及全形冒號等特殊字元的檔案時，會因為 Unicode 的 NFD (macOS 拆解字元) 與 NFC (Linux/GitHub Pages 預期字元) 的編碼標準不同，外加全形特殊符號的網址解析問題，造成部署後網頁讀取回傳 404。
- **解決方案**：將 PDF 檔案一律重命名為乾淨且簡短的英文名稱（如 `Chen_ISACA_Global_AI_Pulse.pdf`），同步修正 HTML 中的相對路徑。這樣不僅能省去複雜的 % 百分比編碼、提高 HTML 程式碼的可讀性，也徹底解決了跨平台檔名解碼的 404 錯誤。
